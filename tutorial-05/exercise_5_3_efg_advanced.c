...
double shape_area(struct shape shape) {
    if (shape.type == RECTANGLE) {
        ...
        return 0.5 * shape.object.triangle.height * shape.object.triangle.hypothenuse;
...
struct shape init_triangle(double hypothenuse, double height) {
    struct triangle new_triangle = {hypothenuse, height};
    struct shape new_shape;
